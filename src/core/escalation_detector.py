...
async def analyze(user_message: str, ai_reply: str) -> dict:
    ...
    try:
        # 调用 LLM 进行意图识别，使用低 temperature 保持输出稳定性
        ...
        # P3-fix: 使用正则安全提取 JSON 对象，兼容所有 LLM 模型的输出格式
        ...
        reason = str(data.get("reason_code", "none"))
        reason_label = str(data.get("reason_label", "无"))

        # 二次保险：如果 LLM 判断为 False，但 AI 回复明确提到转接人工，则强制纠正
        if not should_escalate and detect_escalation(ai_reply):
            should_escalate = True
            if reason == "none":
                reason, reason_label = identify_reason(user_message)

        logger.debug(f"LLM 意图识别成功 | should_escalate: {should_escalate} | reason: {reason}")
        return {
