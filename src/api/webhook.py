...
async def _process_chat(
    ...
    logger.info(
        f"处理消息 | user: {user_id} | platform: {platform} | "
        f"消息: {message[:40]} | RAG命中: {len(retrieved_docs)} 条"
    )
    ...
    # === 升级检测 + 入库 ===
    escalation_result = analyze_escalation(message, reply)
...
async def pdd_webhook(req: PddWebhookRequest, db: DBSession = Depends(get_db)):
    """
...
async def health(db: DBSession = Depends(get_db)):
    ...
    P1-6: 新增 LLM ping 探测（发送单条极短消息验证 LLM API 可用性）
    """
    rag = get_rag_engine()
